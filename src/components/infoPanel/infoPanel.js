...
function InfoPanel({changeStatePanelActivated, infoPanelData}) {
    ...
    return infoData.map(data => 
        ...
        <Nav.Item key={data.id}>
          <Nav.Link eventKey={data.id}>
            <p className="desktop-body_column1_panel-text"> 
              {data.question}
            </p>
            </Nav.Link>
      </Nav.Item> 
  ...
  const mapNavItemAnswerDesktop = (infoData) => {
    return infoData.map(data =>
        ...
        <Tab.Pane eventKey={data.id} key={data.question} className="desktop-body_column2_tab-pane">
          ...
          <p className="desktop-body_column2_panel-title">How does GTS approach this?</p>
          <p className="desktop-body_column2_panel-text">
          ...
          <p className="desktop-body_column2_panel-title">What's happening in detail </p>
          <p className="desktop-body_column2_panel-text">
  ...
  const mapAccordionItems = (infoData) => {
    return infoData.map(data =>
      ...
      <Accordion.Header>
        <p className="accordion_panel-title">
          {data.question}
        </p>
        </Accordion.Header>
      <Accordion.Body className="accordion-body">
        ...
        <p className="accordion-body_text">
            {data.the_simple_answer}
        </p>
    ...
    <div className="info-panel" data-testid="info_wrapper">
      ...
      <div className="info-panel_title-section">
        ...
        <Stack direction="horizontal" gap={3}>
          ...
          <div>
            <img className="info-panel_exit-button" data-testid="info_wrapper_exit_img" src={X} onClick={changeStatePanelActivated}/>
          </div>
        </Stack>
      ...
      <div className="info-panel_desktop-body">
        ...
        <Tab.Container id="left-tabs-example" defaultActiveKey={firstInfoPanel}>
          <Row>
            <Col md={3}  style={{backgroundColor: 'black', height: '78vh', overflow: 'scroll', borderRadius: '25px'}} className="desktop-body_column1">
            ...
            <Col md={9} className="desktop-body_column2" style={{height: '78vh', overflow: 'scroll', position: 'top', borderRadius: '25px'}}>
              <Tab.Content>
